...
import { fetchBatchData } from "@/api/resilienceAssess/public"; // Mock API for fetching list data
...
export default defineComponent({
  ...
  setup() {
    ...
    const getStatusLabel = (item: ListItem) => {
      if (item.rresult >= thresholds.high) {
        return "Safe";
      } else if(item.rresult < thresholds.low) {
        return "Danger";
      } else {
        return "Warning"
      }
