        ...
        <div class="flex justify-center w-full">
          ...
          <div v-else-if="chartData && chartData.length > 0" class="table-container w-[90%]">
            <el-table 
              ...
              :max-height="500"
            >
              <el-table-column prop="device_id" label="设备ID" width="120" fixed />
              <el-table-column label="设备名称" width="120" fixed>
                <template #default="scope">
                  {{ getDeviceName(scope.row.device_id) }}
                </template>
              </el-table-column>
              <el-table-column prop="time" label="时间" width="180" />
              <el-table-column prop="data" label="数据值" width="120" />
              <el-table-column prop="direction" label="方向" width="120">
                <template #default="scope">
                  {{ getDirectionLabel(scope.row.direction) }}
                </template>
              </el-table-column>
              <el-table-column prop="min" label="最小阈值" width="120" />
              <el-table-column prop="max" label="最大阈值" width="120" />
            </el-table>
          </div>
