  ...
  <div class="page-shell">
    ...
    <UForm :state="formState" class="page-body">
      ...
      <section class="panel">
        ...
        <div class="note-card">
          当前页面只保留一套人工生效阈值。页面显示的上下限由底层 `offset / limit` 自动换算：
          上限 = offset + limit，下限 = offset - limit。
        </div>
        ...
        <div v-for="channel in visibleChannels" :key="channel.key" class="channel-card">
          ...
          <div class="limit-grid">
            ...
            <article class="limit-panel ghost-panel">
              <div class="panel-title">
                <h4>Agent 推荐</h4>
                <span>前端占位</span>
              </div>
              <div class="placeholder-copy">
                <p>当前版本不接入 Agent 推荐值，不参与实际阈值计算，也不保存到数据库。</p>
                <p>后续如果需要扩展，再单独接入推荐接口和存储结构。</p>
              </div>
            </article>
          </div>
        </div>
        ...
        <div class="action-bar">
          ...
          <UButton color="gray" disabled>应用 Agent 推荐</UButton>
        </div>
      </section>
      ...
      <section class="panel">
        ...
        <div class="note-card">
          精简版系统不再手动填写三套独立阈值，也不在浏览器中配置邮件间隔。当前规则固定为：
          一级预警 = 生效阈值的 100%，二级预警 = 75%，三级预警 = 50%。邮件发送频率先由后端统一硬编码控制。
        </div>
...
const applyManualLimits = async () => {
  ...
  try {
    const channelCalibrations = visibleChannels.value.map((channel) => {
      ...
      if (!parsed) {
        throw new Error(`${channel.label} 的上下限输入无效，请确认上限不小于下限。`);
      }
