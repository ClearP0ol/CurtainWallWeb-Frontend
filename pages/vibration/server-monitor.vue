<template>
  <!-- 作为振动监测中心子路由，外层布局由 pages/vibration.vue 提供，此处仅内容区 -->
          ...
          <el-tab-pane label="资源总览" name="overview">
            ...
            <el-row v-if="overview" :gutter="16">
              <el-col :xs="24" :sm="8">
                <el-card
                  shadow="hover"
                  :class="['metric-card', statusClass(overview.cpu?.status, 'cpu')]"
                >
              ...
              <el-col :xs="24" :sm="8">
                <el-card
                  shadow="hover"
                  :class="['metric-card', statusClass(overview.memory?.status, 'mem')]"
                >
              ...
              <el-col :xs="24" :sm="8">
                <el-card
                  shadow="hover"
                  :class="['metric-card', statusClass(overview.disk?.status, 'disk')]"
                >
          ...
          <el-tab-pane label="阈值设置" name="thresholds">
            <el-form v-if="thresholdForm" label-width="140px" class="max-w-md">
              <el-form-item label="内存阈值 (%)">
                <el-input-number
                  v-model="thresholdForm.memory_percent"
                  :min="0"
                  :max="100"
                  :step="0.1"
                />
              ...
              <el-form-item label="磁盘阈值 (%)">
                <el-input-number
                  v-model="thresholdForm.disk_percent"
                  :min="0"
                  :max="100"
                  :step="0.1"
                />
              ...
              <el-form-item label="CPU 阈值 (%)">
                <el-input-number
                  v-model="thresholdForm.cpu_percent"
                  :min="0"
                  :max="100"
                  :step="0.1"
                />
              ...
              <el-form-item>
                <el-button type="primary" :loading="savingT" @click="saveThresholds">保存</el-button>
              </el-form-item>
            </el-form>
        ...
        <el-card shadow="hover">
          ...
          <el-descriptions v-if="summary" :column="2" border size="small" class="mb-4">
            ...
            <el-descriptions-item label="最近周期">
              <span
                v-if="summary.latest_cycle"
              >{{ summary.latest_cycle.status }} · {{ summary.latest_cycle.trigger }} ·
                耗时 {{ summary.latest_cycle.duration_ms ?? "—" }}ms</span>
              <span v-else>—</span>
            </el-descriptions-item>
          </el-descriptions>
          <div v-else class="text-sm text-gray-500">暂无摘要数据（请确认监控服务已启动，且已配置 NUXT_SERVER_MONITOR_TARGET）</div>
        </el-card>
  </div>
</template>
...
async function loadThresholds() {
  try {
    const res: any = await api.getSystemThresholds();
    const s = res?.system;
    if (s) {
      thresholdForm.value = {
        ...
        enable_cpu: !!s.enable_cpu
      };
      thresholdMeta.value = res?.meta || null;
...
async function saveThresholds() {
  ...
  try {
    ...
    notifyError(e, "保存阈值失败，请联系后端查看日志");
  } finally {
    savingT.value = false;
  }
}
